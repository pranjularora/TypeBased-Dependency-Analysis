...
#include "DepAnal.h"
#include "../Display/strongComponentDisp.h"
...
bool CodeAnalysisExecutive::ProcessCommandLine(int argc, char* argv[])
{
	...
	try {
		path_ = FileSystem::Path::getFullFileSpec(argv[1]); std::cout << "\n  Path to the directory tree containing files to analyze: --> " << path_;
		bool checkXML = false, checkXML_SC = false;
		...
		for (int i = 2; i < argc; ++i)
		{
			...
			if (argument == "/dp")
			{
				...
				if (argument.find("*.xml") == std::string::npos)
				{
					xmlPath = argument; std::cout << "\n  Dependency File XML will be created with Name: --> " << argument; checkXML = true;
				}
			}
			else if (argument == "*.h" || argument == "*.cpp" || argument == "*.cs") patterns_.push_back(argv[i]);
			else if (argument == "/sc")
			{
				std::string argument = std::string(argv[i + 1]);
				if (argument.find("*.xml") == std::string::npos)
				{ xmlPathStrongComp = argument; std::cout << "\n  Strong Component XML will be created with Name: --> " << argument; checkXML_SC = true; }
			}
		}
		if (!checkXML) std::cout << "\n  No command Line Argument provided for Dependency File XML, using Default File Name: --> " << returnXmlPath();
		if (!checkXML_SC) std::cout << "\n  No command Line Argument provided for Strong Component XML, using Default File Name: --> " << returnXmlPath_SC();
		if (patterns_.size() == 0)
		{	showUsage(); return false; 	}
		else
		{
			std::cout << "\n  List of file patterns to match for selection of files to analyze: -->";
			for (auto args : patterns_)
				std::cout << " " << args;
		}
	}
...
std::string CodeAnalysisExecutive::returnXmlPath()
{
	return xmlPath;
}

std::string CodeAnalysisExecutive::returnXmlPath_SC()
{
	return xmlPathStrongComp;
}
int CodeAnalysisExecutive::ProfessorCodeInitialization(CodeAnalysisExecutive& exec, int argc, char* argv[])
{
...
void CodeAnalysisExecutive::displayRequirement4Style()
{
	std::cout << "\n\n\n========================================================================";
	std::cout << "\n  Displaying Requirement 4: =============>";

	std::cout << "\n\n  A Type Analysis Package named- TypeTable\n  that identifies all of the types defined in a specified file\n";
	std::cout << "========================================================================\n";
	std::cout << "\n  <<<----------Starting Type Analysis: ---------->>>";
}

void CodeAnalysisExecutive::displayRequirement5Style()
{
	std::cout << "\n\n\n========================================================================";
	std::cout << "\n  Displaying Requirement 5: =============>";

	std::cout << "\n\n  A Dependency Analysis Package named- Dependency Table\n  that identifies all of the dependencies between files in a specified file collection\n";
	std::cout << "========================================================================\n";
	std::cout << "\n  <<<----------Starting Dep Analysis: ---------->>>";
}
void CodeAnalysisExecutive::displayRequirement6Style()
{
	std::cout << "\n\n\n=============================================================================";
	std::cout << "\n  Displaying Requirement 6: =============>";

	std::cout << "\n\n  A Strong Component Package named- StrongComponents is created..\n";
	std::cout << "  Graph(created using Graph class) is populated from Dependency Analysis results.  \n  \n";
	std::cout << "=============================================================================\n";
	std::cout << "\n  <<<----------Starting Strong Component Analysis: ---------->>>";
}
void CodeAnalysisExecutive::displayRequirement8Style()
{
	std::cout << "\n\n\n=============================================================================";
...
void CodeAnalysisExecutive::displayRequirement9Style()
{
	std::cout << "\n\n\n=============================================================================";
	std::cout << "\n  Displaying Requirement 9: =============>";
	std::cout << "\n An automated unit test suite that demonstrates all the requirements has been created by using Executive.cpp in the code Analyser project and can be verified by looking into the code.";
	std::cout << "\n=============================================================================\n";

}
void CodeAnalysisExecutive::displayRequirement1Style()
{
	std::cout << "\n\n\n=============================================================================";
	std::cout << "\n  Displaying Requirement 1: =============>";
	std::cout << "\n An automated unit test suite that demonstrates all the requirements has been created by using Executive.cpp in the code Analyser project and can be verified by looking into the code.";
	std::cout << "\n=============================================================================\n";

}
void CodeAnalysisExecutive::displayRequirement2Style()
{
	std::cout << "\n\n\n=============================================================================";
	std::cout << "\n  Displaying Requirement 2: =============>";
	std::cout << "\n used the C++ standard library's streams for all I/O and new and delete for all heap-based memory management, this can be verified by looking into the code.";
	std::cout << "\n=============================================================================\n";

}
void CodeAnalysisExecutive::displayRequirement3Style()
{
	std::cout << "\n\n\n=============================================================================";
	std::cout << "\n  Displaying Requirement 3: =============>";
	std::cout << "\n C++ packages has been created as described in the Purpose section";
	std::cout << "\n Single Responsibility Packages has been created for-";
	std::cout << "\n  - TypeAnalysis \n  - Dependency Analysis  \n  - Strong Component";
	std::cout << "\n All these can be verified by looking into the code.";
	std::cout << "\n=============================================================================\n";
...
int main(int argc, char* argv[])
{
	...
	try {
		...
		std::cout << "\n--Files to test-- Give Path, include a space, and type either or all [*.cpp, *.h, *.cs], as Analysis can be done on these \n";
		std::cout << "\n--Use / dp \"Dependency XML Name\" and /sc \"StrongComponent XML Name, else default names will be used\n";
		exec.displayRequirement8Style();
		...
		graph crtGraph;
		StrongComponentDisplay displStrongComponents;
		DependencyTableDisp displDepTable;
		TypeTableDisp dispTypeTable;


		TypeAnal ta(typeTable, depTable);
		exec.displayRequirement4Style();
		ta.doTypeAnal();

		dispTypeTable.displayTypeTable_Tab(typeTable);
		dispTypeTable.displayTypeTable(typeTable);

		exec.displayRequirement5Style();

		ta.doDepAnal();
		displDepTable.displayDependencyTable(depTable);

		exec.displayRequirement6Style();

		StrongComponents strngCmpnts;
		strngCmpnts.CreateGraph(depTable);
		strngCmpnts.SCC();
		displStrongComponents.displayStrongComponents(strngCmpnts);

		Persist<StrData> persist;
		persist.getData(depTable);
		persist.saveToNoSqlDB();
		persist.displayNoSQLDB();
		persist.persist(exec.returnXmlPath());

		Persist<StrData> persist1;
		persist1.getDataStrongComp(strngCmpnts);
		persist1.saveToNoSqlDB_StrongComp();
		persist1.displayNoSQLDB();
		persist1.persist(exec.returnXmlPath_SC());
		exec.displayRequirement9Style();
		exec.displayRequirement1Style();
		exec.displayRequirement2Style();
		exec.displayRequirement3Style();
